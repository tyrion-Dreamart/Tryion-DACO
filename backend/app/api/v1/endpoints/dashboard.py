...
async def get_summary(db: DBDep, current_user: CurrentUser):
    ...
    all_inv_r = await db.execute(
        select(Invoice).where(
            Invoice.status.in_([InvoiceStatus.PENDING, InvoiceStatus.PARTIALLY_PAID, InvoiceStatus.PAID, InvoiceStatus.OVERDUE])
        )
    )
    ...
    facturado = sum(
        ...
        if i.status in [InvoiceStatus.PENDING, InvoiceStatus.PARTIALLY_PAID, InvoiceStatus.PAID]
    )
    cobrado = sum(
    ...
    por_cobrar = sum(
        ...
        if i.status in [InvoiceStatus.PENDING, InvoiceStatus.PARTIALLY_PAID]
    )
    vencido = sum(
        to_mxn(i.balance, i) for i in all_inv
