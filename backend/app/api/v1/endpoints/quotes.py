...
def _calc_totals(quote: Quote, items: list) -> None:
...
async def list_quotes(
    ...
    for q in quotes:
        ...
        resp = QuoteResponse(
            ...
            attention_name=q.attention_name,
            attention_area=q.attention_area,
            ...
            advance_pct=float(q.advance_pct) if q.advance_pct else None,
            notes=q.notes,
...
async def create_quote(payload: QuoteCreate, db: DBDep, current_user: CurrentUser):
    ...
    resp = QuoteResponse(
        ...
        advance_pct=float(quote.advance_pct) if quote.advance_pct else None,
        notes=quote.notes,
        subtotal=float(quote.subtotal),
        iva_amount=float(quote.iva_amount),
        total=float(quote.total),
        pdf_url=quote.pdf_url,
        extracted_by_ai=quote.extracted_by_ai,
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
        items=[],
    )
    ...
    if cl:
        resp.client_name = cl.legal_name
    return resp
...
async def get_quote(quote_id: str, db: DBDep, current_user: CurrentUser):
    ...
    q_items = list(items_result.scalars().all())
...
async def update_quote(
    ...
    if payload.items is not None:
        ...
        items_res = await db.execute(select(QuoteItem).where(QuoteItem.quote_id == quote_id))
        updated_items = list(items_res.scalars().all())

    _calc_totals(quote, updated_items)
    await db.flush()
